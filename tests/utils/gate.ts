...
type GateDefMethod<K extends string> = {
  method: Partial<Record<K, (page: Page) => Promise<unknown>>>;
};
type GateDefExtra = {
...
// Use Types -----------------------------------------------------------------------------------------------------
type GateUseMisc = {
  goto: Page["goto"] extends (url: string, options: infer O) => Promise<infer R>
    ? (options?: O) => Promise<R>
    : never;
};
// prettier-ignore
...
type GateUse<D extends GateDef<string>> = GateUseMethod<D["method"]> &
  GateUseMisc &
  GateUseExtra<D["extra"]>;
// prettier-ignore
...
// Methods -------------------------------------------------------------------------------------------------------
export const defineGate =
  <K extends string, D extends GateDefMap<K>>(_: K, defMap: D) =>
  (page: Page) => {
