...
import { type FrameLocator, type Page, type Locator, type TestStepInfo, test } from "@playwright/test";
...
export function rootType<R extends RootType>() {
  return <T extends AuthView<R>>(authViewDef: T) => {
    return (root: R, profile: Profile) => {
      ...
      const step = async (name: string, fn: (expect: AuthUseView<T>, info: TestStepInfo) => Promise<void>) => {
        return test.step(name, async (info) => {
          await fn(base, info);
        });
      };
